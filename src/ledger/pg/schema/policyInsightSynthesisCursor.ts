...
export const policyInsightSynthesisCursor = regimeEngine.table(
  ...
  (t) => [
    check(
      ...
      sql`${t.lastProcessedReceiptId} >= 0 AND (${t.targetReceiptId} IS NULL OR ${t.targetReceiptId} >= 0) AND ${t.attemptCount} >= 0 AND (${t.leaseExpiresAtUnixMs} IS NULL OR ${t.leaseExpiresAtUnixMs} >= 0) AND (${t.nextAttemptAtUnixMs} IS NULL OR ${t.nextAttemptAtUnixMs} >= 0) AND ${t.updatedAtUnixMs} >= 0`
    ),
    check(
      "chk_synthesis_cursor_lease_coherence",
      sql`(${t.leaseOwner} IS NULL AND ${t.targetReceiptId} IS NULL AND ${t.leaseExpiresAtUnixMs} IS NULL) OR (${t.leaseOwner} IS NOT NULL AND ${t.targetReceiptId} IS NOT NULL AND ${t.leaseExpiresAtUnixMs} IS NOT NULL)`
    ),
    check(
