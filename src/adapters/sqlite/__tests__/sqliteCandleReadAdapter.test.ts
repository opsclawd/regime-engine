...
import { createSqliteCandleReadAdapter } from "../sqliteCandleReadAdapter.js";
...
describe("sqliteCandleReadAdapter", () => {
  let store: ReturnType<typeof createLedgerStore>;
  let adapter: ReturnType<typeof createSqliteCandleReadAdapter>;

  beforeEach(() => {
    store = createLedgerStore(":memory:");
    adapter = createSqliteCandleReadAdapter(store);
  });
  ...
  describe("getCandlesForFeedWindow", () => {
    it("returns only the complete feed key within inclusive bounds in ascending order", async () => {
      insertRow(seedRow({ unixMs: 1 * FIFTEEN_MIN_MS }));
      insertRow(seedRow({ unixMs: 2 * FIFTEEN_MIN_MS }));
      insertRow(seedRow({ unixMs: 3 * FIFTEEN_MIN_MS }));
      insertRow(seedRow({ unixMs: 4 * FIFTEEN_MIN_MS }));
      insertRow(seedRow({ unixMs: 5 * FIFTEEN_MIN_MS }));
      insertRow(
        seedRow({
          symbol: "OTHER/USD",
