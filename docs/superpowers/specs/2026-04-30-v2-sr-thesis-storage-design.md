...
- Duplicate thesis identities inside the same request are rejected before any writes. The duplicate key is `(source, symbol, briefId, asset, sourceHandle)`. A duplicate returns `400 VALIDATION_ERROR`, even when the duplicate thesis payloads are identical.
- `brief.sourceRecordedAtIso` is a required nullable field.
...
1. Validate the full request, including rejection of duplicate `(source, symbol, briefId, asset, sourceHandle)` thesis identities before any writes.
...
`insertBrief` computes row hashes from `request` using the v2 contract hash helper, then performs all row writes and conflict checks inside one transaction. `getCurrent` finds the newest brief row for `(symbol, source)` ordered by `captured_at_unix_ms DESC, id DESC`, then returns all rows for that `(source, symbol, brief_id)` ordered by `id ASC`.
...
- `400 VALIDATION_ERROR` for malformed input, including duplicate thesis identities inside one request, or `400 UNSUPPORTED_SCHEMA_VERSION` from the v2 parser.
...
- `503 SERVICE_UNAVAILABLE` when the Postgres-backed store is unavailable after auth succeeds.
...
- Reject missing `source`, `symbol`, `brief`, empty `theses`, missing required thesis fields, duplicate thesis identities in the same request, bad ISO timestamp strings, and unknown keys.
- Prove exact timestamp strings survive parsing and hash input without normalization.
- Prove `sourceRecordedAtIso: null`, `collectedAt: null`, and `publishedAt: null` are accepted and preserved in hash input.
...
- Row-to-wire reconstruction preserves arrays, nullable fields, exact non-null timestamp strings, and null timestamp values.

HTTP tests without `DATABASE_URL`:

- `POST /v2/sr-levels` returns `503 SERVICE_UNAVAILABLE` with `schemaVersion: "2.0"` only when `OPENCLAW_INGEST_TOKEN` is configured and the request includes a valid `X-Ingest-Token`.
- `POST /v2/sr-levels` still returns `500 SERVER_MISCONFIGURATION` when `OPENCLAW_INGEST_TOKEN` is missing, even without `DATABASE_URL`.
...
- Duplicate thesis identities inside one request return `400 VALIDATION_ERROR` with `schemaVersion: "2.0"` before any writes.
...
- GET success returns the latest brief and exact stored thesis fields, including exact round-trip of non-null timestamp strings and null timestamp values.
...
- Timestamp fields round-trip as exact inbound strings when non-null and as `null` when null.
- Duplicate thesis identities inside one request return `400 VALIDATION_ERROR` before any writes.
